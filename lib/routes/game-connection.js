const hslToColor = require("../utils/color").hslToColor
const Map = require("../map/map").Map;
const VERSION = 1;
const directions = require("../../shared_lib/constants").directions;
...
module.exports = function gameConnection (app, io, config) {
    ...
    function setSnakeBody(socket, slot, direction) {
        slot.setSolid(true);
        slot.setUsed(true);
    ...
    sio.on('connection', function (socket) {
        ...
        socket.on('pre-start', function(version) {
            ...
            var directionArr = Object.keys(directions).map(function(key) {
                return directions[key]
            })
            
            var selectedDirection = directionArr[Math.floor(directionArr.length * Math.random())]
            
            var initialSlot = map
            .getChunk(chunkIndex.x    , chunkIndex.y    )
            .getSlot(chunkIndex.offsetX, chunkIndex.offsetY);
            
            setSnakeBody(socket, initialSlot, selectedDirection)
        ...
        socket.on('move', function(position, addLength, direction) {
            ...
            setSnakeBody(socket, secondSlot, direction)
        ...
        socket.on('direction', function (position, direction) {
            var chunkIndex = map.getChunkIndex(position.x, position.y);
            var chunk = map.getChunk(chunkIndex.x, chunkIndex.y);
            var slot = chunk.getSlot(chunkIndex.offsetX, chunkIndex.offsetY);
            
            if (!slot.isSolid() || slot.owner !== socket.playerId) {
                return;
            }
